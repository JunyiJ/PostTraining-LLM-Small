...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for idx in tqdm(range(0, len(train_samples), BATCH_SIZE), desc=f"epoch {epoch}", leave=False):
        ...
        ratio = torch.exp(log_probs_new - log_probs_old) * answer_mask 
        ...
        with torch.no_grad():
            ...
            rewards = kl_penalty.detach().clone() * answer_mask
        ...
        with torch.enable_grad():
            ...
            policy_loss = -(torch.min(surr1, surr2) * answer_mask).sum() / (answer_mask.sum() + 1e-8)
            ...
            value_loss = 0.5 * (torch.max(v_loss_1, v_loss_2) * answer_mask).sum() / (answer_mask.sum() + 1e-8)
            ...
            if idx == 0:
                # Lightweight debug for first batch only
                for q, a, t in zip(questions, golds, res.get("text")):
                    print(q)
                    print(a)
                    print(t)
                    print("===")
            ...
            print(
                {
                    ...
                    "ratio_mean": ratio.sum() / (answer_mask.sum() + 1e-8),
                    "adv_mean": adv_mean,
                    "reward_mean": (rewards * answer_mask).sum()/(answer_mask.sum() + 1e-8),
        ...
        with torch.no_grad():
            valid_kl = (kl_divergence * answer_mask).sum() / (answer_mask.sum() + 1e-8)
        ...
        del values_new, values_old, new_values, old_values
